...
float fine_delay_time(
    float *samples,
    int num_samples,
    uint8_t *tones,
    float delay_start,
    float delay_stop,
    float freq, bool print_me)
{
    ...
    //
    // Reference amplitudes and phases.
    //
    ...
    //
    // Generate the reference waveform only once.
    //
    ...
    //
    // Convert delay limits to samples.
    //
    const int delay_first =
        (int)std::lround(delay_start * sample_rate);

    const int delay_last =
        (int)std::lround(delay_stop * sample_rate);

    //
    // Search for the maximum correlation.
    //
    double best_score = -1.0;
    int best_delay = delay_first;


    printf("Searching for fine delay from %.6f to %.6f seconds waveform_samples=%d  num_samples=%d...\n", delay_start, delay_stop, waveform_samples, num_samples );
    for (int delay = delay_first;
         delay <= delay_last;
         ++delay)
    {
        ...
        for (int k = 0; k < 79; k++)
        {
            ...
            const int input_start = delay + k * block;

            if (input_start >= num_samples)
                break;

            const int count =
                std::min(block, num_samples - input_start);

            for (int n = 0; n < count; n++)
            {
                const float x = samples[input_start + n];

                ci += x * reference_i[k * block + n];
                cq += x * reference_q[k * block + n];
            }
        ...
        if(print_me)
            printf("delay = %.6f seconds (score = %.6f)  best_delay = %.6f seconds (best_score = %.6f)\n", 
                (float)delay/(float)sample_rate, score, (float)best_delay/(float)sample_rate, best_score);
    }

    return (float)best_delay / sample_rate;
}
    ...
    int cand_to_print)
{
     ...
     * delay0 +/- 300 ms.
     *
     * We calculate the correlation score for every sample in
